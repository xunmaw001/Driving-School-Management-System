	...
	<div class="detail-preview" :style='{"width":"1200px","padding":"0 0 20px","margin":"20px auto 0","position":"relative"}'>
		<div class="attr" :style='{"padding":"20px","borderColor":"#52b646","borderRadius":"12px","flexWrap":"wrap","background":"#fff","borderWidth":"3px","display":"flex","position":"relative","borderStyle":"dotted dashed solid double","justifyContent":"center"}'>
			...
			<div class="info" :style='{"width":"50%","padding":"10px","margin":"0 2% 0 0px","background":"#fff","order":"2"}'>
				...
				<div class="item" :style='{"border":"1px solid #88d67e","padding":"6px 10px","boxShadow":"inset 0px 0px 48px 0px #def5db","margin":"0 0 10px 0","borderRadius":"4px","background":"#fff","display":"flex","justifyContent":"spaceBetween"}'>
					<div class="lable" :style='{"padding":"0 10px","color":"#333","textAlign":"right","background":"none","width":"120px","fontSize":"14px","lineHeight":"40px","minWidth":"120px"}'>可约时间</div>
				...
				<div class="btn" :style='{"padding":"10px 0","flexWrap":"wrap","display":"flex"}'>
					<el-button :style='{"border":"0","cursor":"pointer","padding":"0 10px","margin":"0 5px 0 0","outline":"none","color":"#fff","borderRadius":"4px","background":"radial-gradient(circle, rgba(132,218,110,1) 0%, rgba(63,187,33,1) 81%, rgba(82,182,70,1) 100%)","width":"auto","lineHeight":"40px","fontSize":"14px","height":"40px"}' v-if="isAuth('jiaxiaocheliang','预约')" @click="onAcross('yuyuexinxi','','','')" type="warning">预约</el-button>
				</div>
			</div>
		...
		<el-tabs class="detail" :style='{"boxShadow":"0 1px 6px 0 rgba(0, 0, 0, .1)","borderColor":"#52b646","borderRadius":"12px","borderStyle":"solid double solid double","background":"#fff","borderWidth":"3px"}' v-model="activeName" type="border-card">
			<el-tab-pane label="车辆信息" name="first">
				<div v-html="detail.cheliangxinxi"></div>
			</el-tab-pane>
  ...
  export default {
    ...
    methods: {
      ...
      thumbsupOrCrazily(type) {
        ...
        this.$http.post('jiaxiaocheliang/update', this.detail).then(res => {});
      },
    ...
    getThumbsupOrCrazilyStatus() {
        if(localStorage.getItem("Token")) {
            this.$http.get('storeup/list', {params: {page: 1, limit: 1, type: 21, refid: this.detail.id, tablename: 'jiaxiaocheliang', userid: localStorage.getItem('userid')}}).then(res => {
              if (res.data.code == 0 && res.data.data.list.length > 0) {
            ...
            this.$http.get('storeup/list', {params: {page: 1, limit: 1, type: 22, refid: this.detail.id, tablename: 'jiaxiaocheliang', userid: localStorage.getItem('userid')}}).then(res => {
              if (res.data.code == 0 && res.data.data.list.length > 0) {
      ...
      getDiscussList(page) {
        this.$http.get('discussjiaxiaocheliang/list', {params: {page, limit: this.pageSize, refid: this.detail.id}}).then(res => {
          if (res.data.code == 0) {
      ...
      submitForm(formName) {
        ...
        this.$refs[formName].validate((valid) => {
          if (valid) {
            ...
            this.$http.post('discussjiaxiaocheliang/add', this.form).then(res => {
              if (res.data.code == 0) {
                this.form.content = '';
                this.getDiscussList(1);
                this.$message({
