...
<div>
    ...
    <el-form :inline="true" :model="formSearch" class="list-form-pv" :style='{"padding":"12px 10px 20px","margin":"20px 0 0 0","borderColor":"#52b646","alignItems":"center","borderRadius":"12px","flexWrap":"wrap","background":"#eff9ee","borderWidth":"4px","display":"flex","width":"100%","borderStyle":"solid dashed dotted double","height":"auto"}'>
      <el-form-item :style='{"margin":"0 10px"}'>
		<div class="lable" v-if="true" :style='{"width":"auto","padding":"0 10px","lineHeight":"42px","display":"inline-block"}'>学员账号</div>
        <el-select v-model="formSearch.xueyuanzhanghao" placeholder="请选择学员账号" :clearable="true">
          <el-option v-for="(item, index) in xueyuanzhanghaoOptions" :key="index" :label="item" :value="item"></el-option>
        </el-select>
      </el-form-item>
      <el-form-item :style='{"margin":"0 10px"}'>
	    <div class="lable" v-if="true" :style='{"width":"auto","padding":"0 10px","lineHeight":"42px","display":"inline-block"}'>驾驶证类型</div>
        <el-input v-model="formSearch.jiashizhengleixing" placeholder="驾驶证类型" clearable></el-input>
      </el-form-item>
      <el-form-item :style='{"margin":"0 10px"}'>
		<div class="lable" v-if="true" :style='{"width":"auto","padding":"0 10px","lineHeight":"42px","display":"inline-block"}'>是否合格</div>
        <el-select v-model="formSearch.shifouhege" placeholder="请选择是否合格" :clearable="true">
          <el-option v-for="(item, index) in shifouhegeOptions" :key="index" :label="item" :value="item"></el-option>
        </el-select>
      </el-form-item>
	  ...
	  <el-button v-if="isAuth('xueyuanchengji','新增')" :style='{"cursor":"pointer","border":"0","padding":"0px 15px","margin":"0 10px 0 0","outline":"none","color":"#fff","borderRadius":"4px","background":"radial-gradient(circle, rgba(255,215,169,1) 0%, rgba(255,164,58,1) 100%)","width":"auto","fontSize":"14px","lineHeight":"42px","height":"42px"}' type="primary" @click="add('/index/xueyuanchengjiAdd')"><i v-if="true" :style='{"color":"#fff","margin":"0 10px 0 0","fontSize":"14px"}' class="el-icon-circle-plus-outline"></i>添加</el-button>
    </el-form>
...
<script>
  export default {
    ...
    created() {
      ...
      this.$http.get('option/xueyuan/xueyuanzhanghao').then(res => {
        if (res.data.code == 0) {
    ...
    methods: {
      ...
      getList(page, fenlei, ref = '') {
        ...
        if (this.formSearch.jiashizhengleixing != '') searchWhere.jiashizhengleixing = '%' + this.formSearch.jiashizhengleixing + '%';
        if (this.formSearch.shifouhege != '') searchWhere.shifouhege = this.formSearch.shifouhege;
        this.$http.get('xueyuanchengji/list', {params: Object.assign(params, searchWhere)}).then(res => {
          if (res.data.code == 0) {
      ...
      toDetail(item) {
        this.$router.push({path: '/index/xueyuanchengjiDetail', query: {detailObj: JSON.stringify(item)}});
      },
    }
  }
</script>
